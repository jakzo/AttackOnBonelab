...
namespace AttackOnBonelab
{
    public static class Geometry
    {
        ...
        {
            var gameObject = CreatePrefab(name);
            var meshRenderer = AddUnclosedCylinderMesh(
                ref gameObject,
                radius,
                segments,
                top,
        ...
        public static GameObject CreatePrefabSphere(
            string name,
            Color color,
            float radius,
            int subdivisions
        ...
        {
            var gameObject = CreatePrefab(name);
            var meshRenderer = AddSphereMesh(
                ref gameObject,
                radius,
                subdivisions
            );
        ...
        public static GameObject CreatePrefabLine(
            string name,
            Color color,
            float width
        )
        ...
        public static MeshRenderer AddSphereMesh(
            ref GameObject gameObject,
            float radius,
            int subdivisions
        )
        {
            var meshFilter = gameObject.AddComponent<MeshFilter>();
            meshFilter.mesh.Clear();
            var (vertices, triangles, normals) = IcoSphere.Create(
                radius,
                subdivisions
            );
        ...
        )
        {
            var meshFilter = gameObject.AddComponent<MeshFilter>();
            meshFilter.mesh.Clear();
            var circleCoords = Enumerable
                .Range(0, segments)
                .Select(i =>
                {
                    var angle =
                        (float)i / (float)segments * (float)System.Math.PI * 2f;
                    return new Vector2(
                        Mathf.Sin(angle) * radius,
                        Mathf.Cos(angle) * radius
                    );
                });
            meshFilter.mesh.vertices = new[] { bottom, top }
                .SelectMany(
                    y =>
                        circleCoords.Select(
                ...
                .ToArray();
            meshFilter.mesh.triangles = Enumerable
                .Range(0, segments)
                .SelectMany(idx =>
                {
        ...
        public static class IcoSphere
        {
            ...
            public static (Vector3[], int[], Vector3[]) Create(
                float radius,
                int subdivisions
            )
            {
                ...
                var normals = vertices
                    .Select(vertex => vertex.normalized)
                    .ToArray();
